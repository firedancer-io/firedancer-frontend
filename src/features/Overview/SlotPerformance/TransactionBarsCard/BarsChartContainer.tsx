...
export default function BarsChartContainer() {
  ...
  const maxTs = useMemo(() => {
    ...
    return (
      max(
        query.response.transactions.txn_mb_end_timestamps_nanos.map((ts) =>
          Number(
            ts - (query.response?.transactions?.start_timestamp_nanos ?? 0n),
          ),
        ),
      ) ?? 0
    );
  ...
  return (
    <Flex direction="column" height="100%" gap="2" key={slot}>
      <Flex gap="2">
        <CardHeader text="Bank&nbsp;Txns" />
        <ChartControls
