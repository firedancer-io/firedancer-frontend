...
export function timelinePlugin(
  ...
  let maxCuIncome: Record<number, number> = {};
  ...
  function setMaxCuIncome() {
    maxCuIncome = getMaxCuIncome(transactionsRef);
  }
  ...
  const opts = {
    ...
    fill: (
      ...
      if (value === FilterEnum.INCOME_CUS) {
        const txnIdx = data[txnIdxSidx][dataIdx] ?? -1;
        const ratio = maxCuIncome[txnIdx] ?? 0;
  ...
  function putBox(
    ...
    if (isAdditionalSeries(sidx)) {
      ...
      if (value === FilterEnum.INCOME_CUS) {
        const txnIdx = chartDataRef.current?.[txnIdxSidx][ix] ?? -1;
        let ratio = maxCuIncome[txnIdx] ?? 0;
      ...
      if (opts.axes)
        uPlot.assign(opts.axes[1], {
          splits: (
            u: {
              bbox: { height: number };
              posToVal: (arg0: number, arg1: string) => number;
            },
            axisIdx: number,
          ) => {
            walk(
