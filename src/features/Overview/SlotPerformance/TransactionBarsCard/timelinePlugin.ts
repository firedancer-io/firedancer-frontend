...
import { ceil, debounce, round } from "lodash";
...
import { barCountAtom, chartFiltersAtom } from "./atoms";
import { TxnState, FilterEnum, stateColors } from "./consts";

const laneWidth = 1;
const laneDistr = SPACE_BETWEEN;

let focusedErrorCode: number;
...
export function getTxnState(
  ...
  // if (focusedErrorCode && txns.txn_error_code[txnIdx] === focusedErrorCode) {
  //   return TxnState.HIGHLIGHT_ERROR;
  // }

  // if (txns.txn_error_code[txnIdx] !== 0) {
  //   return TxnState.ERROR;
  // }
...
const timeSidx = 0;
const txnIdxSidx = 1;
const microblockSidx = 2;

function isTimeSeries(sidx: number) {
  return sidx === timeSidx;
}

function isTxnIdxSeries(sidx: number) {
  return sidx === txnIdxSidx;
}

function isMicroblockSeries(sidx: number) {
  return sidx === microblockSidx;
}

function isAdditionalSeries(sidx: number) {
  return sidx > microblockSidx;
}

function getChartTxnState(
...
function bigIntRatio(numerator: bigint, denominator: bigint, precision = 2) {
...
const syncZoom = debounce(_setZoomRange, 50);
...
function getMax<T extends bigint | number>(
  ...
  return function (
    ...
    // const bigIntScaleMin =
    //   scale?.min != null
    //     ? transactionsRef.current.start_timestamp_nanos +
    //       BigInt(Math.trunc(scale.min))
    //     : undefined;

    // const bigIntScaleMax =
    //   scale?.max != null
    //     ? transactionsRef.current.start_timestamp_nanos +
    //       BigInt(Math.ceil(scale.max))
    //     : undefined;
    ...
    return searchArr.reduce((total, val, txnIdx) => {
      if (
        ...
        return total;
      // TODO: fix
      // if (!transactionsRef.current?.txn_landed?.[i]) return total;
      // if (
      //   bigIntScaleMin !== undefined &&
      //   transactionsRef.current.txn_mb_end_timestamps_nanos[i] < bigIntScaleMin
      // )
      //   return total;
      // if (
      //   bigIntScaleMax !== undefined &&
      //   transactionsRef.current.txn_mb_end_timestamps_nanos[i] > bigIntScaleMax
      // )
      //   return total;
    ...
    // return searchArr.reduce((total, val, i) => {
    //   if (!transactionsRef.current?.txn_landed?.[i]) return total;
    //   if (
    //     bigIntScaleMin !== undefined &&
    //     transactionsRef.current.txn_mb_end_timestamps_nanos[i] < bigIntScaleMin
...
const getMaxFees = getMax((transactions) => transactions.txn_priority_fee, 0n);
const getMaxTips = getMax((transactions) => transactions.txn_tips, 0n);
const getMaxCuConsumed = getMax(
  (transactions) => transactions.txn_compute_units_consumed,
  0,
);
const getMaxCuRequested = getMax(
  (transactions) => transactions.txn_compute_units_requested,
  0,
);
const getMaxCuIncome = getMax((transactions) => {
  return transactions.txn_priority_fee.map((_, i) => {
    if (!transactions.txn_compute_units_consumed[i]) return 0;
    return (
      Number(transactions.txn_priority_fee[i] + transactions.txn_tips[i]) /
      transactions.txn_compute_units_consumed[i]
    );
  });
}, 0);

export function timelinePlugin(
  chartDataRef: MutableRefObject<uPlot.AlignedData | undefined>,
  transactionsRef: MutableRefObject<SlotTransactions | null | undefined>,
): uPlot.Plugin {
  let maxFees = 0n; //; getMaxFees(transactionsRef);
  let maxTips = 0n; // getMaxTips(transactionsRef);
  let maxCuConsumed = 0; // getMaxCuConsumed(transactionsRef);
  let maxCuRequested = 0; // getMaxCuRequested(transactionsRef);
  let maxCuIncome = 0;

  function setMaxFees(scale: uPlot.Scale) {
    maxFees = getMaxFees(transactionsRef, scale);
  }
  function setMaxTips(scale: uPlot.Scale) {
    maxTips = getMaxTips(transactionsRef, scale);
  }
  function setMaxCuConsumed(scale: uPlot.Scale) {
    maxCuConsumed = getMaxCuConsumed(transactionsRef, scale);
  }
  function setMaxCuRequested(scale: uPlot.Scale) {
    maxCuRequested = getMaxCuRequested(transactionsRef, scale);
  }
  function setMaxCuIncome(scale: uPlot.Scale) {
    maxCuIncome = getMaxCuIncome(transactionsRef, scale);
  }

  barCount = 1; //(chartDataRef.current?.length ?? 0) - 1;
  ...
  const opts = {
    ...
    fill: (
      ...
      // const filterColor = filterColors[value as FilterColorEnums];
      // if (filterColor) {
      if (value === FilterEnum.FEES) {
        const txnIdx = data[txnIdxSidx][dataIdx] ?? -1;
        const fees = transactionsRef.current?.txn_priority_fee[txnIdx];
        if (!fees) return ""; // filterColor;
        ...
        const cuIncome =
          Number(
            transactionsRef.current.txn_priority_fee[txnIdx] +
              transactionsRef.current.txn_tips[txnIdx],
          ) / transactionsRef.current.txn_compute_units_consumed[txnIdx];

        if (!cuIncome) return "";

        const ratio = cuIncome / maxCuIncome;
        const ratioScaled = Math.max(Math.min(0.8, ratio * 4), 0.3);
      ...
      // return filterColors[value as FilterColorEnums] ?? "rgba(0, 0, 0, .2)";
    },
    // statesDisplay[seriesIdx][value].fill,
    stroke: (
      data: AlignedData,
      seriesIdx: number,
      dataIdx: number,
  ...
  function putBox(
    ...
    if (isAdditionalSeries(sidx)) {
      if (value === FilterEnum.FEES) {
        const txnIdx = data[txnIdxSidx][ix] ?? -1;
        const fees = transactionsRef.current?.txn_priority_fee[txnIdx];
        if (fees !== undefined) {
          let ratio = 1 / logRatio(Number(maxFees), Number(fees), 1.7);
          // let ratio = logRatio2( Number(fees), Number(maxFees), 1000000);
          // let ratio = bigIntRatio(fees, maxFees, 4);
      ...
      if (value === FilterEnum.TIPS) {
        const txnIdx = chartDataRef.current?.[txnIdxSidx][ix] ?? -1;
        const tips = transactionsRef.current?.txn_tips[txnIdx];
        if (tips !== undefined) {
          // let ratio = bigIntRatio(tips, maxTips, 4);
      ...
      if (value === FilterEnum.INCOME_CUS) {
        ...
        if (cuIncome !== undefined) {
          let ratio = cuIncome / maxCuIncome;
          if (ratio > 0.9) ratio = 0.9;
          if (ratio < 0.1) ratio = 0.1;
          const ratioHgt = hgt * ratio;
          const diff = hgt - ratioHgt;
  ...
  function drawPaths(
    ...
    setMaxFees(u.scales["x"]);
    setMaxTips(u.scales["x"]);
    setMaxCuConsumed(u.scales["x"]);
    setMaxCuRequested(u.scales["x"]);
    setMaxCuIncome(u.scales["x"]);
  ...
  return {
    hooks: {
      setScale: (u, scaleKey) => {
        if (u.scales.x.min == null || u.scales.x.max == null) return;
        syncZoom([Math.trunc(u.scales.x.min), Math.trunc(u.scales.x.max)]);
      },
      init: (u: { root: { querySelector: (arg0: string) => any } }) => {
      ...
      setCursor: (u: uPlot) => {
        if (mode == 1) {
          const val = u.posToVal(u.cursor.left ?? 0, "x");
