...
import { getIncome, isElementFullyInView } from "../../../../../utils";
import { filteredTxnIdxAtom } from "../atoms";
import { txnErrorCodeMap } from "../../../../../consts";
import { containerElAtom, peersAtom } from "../../../../../atoms";
import {
  searchLengthThreshold,
  getSignatureLabelFn,
  desiredScaleTotalMultiplier,
  getIpLabelFn,
  findIpMatch,
...
export default function SearchCommand({ transactions }: SearchCommandProps) {
  ...
  const ipOptionMap = useMemo(() => {
    const ipToName = Object.values(peers).reduce((map, peer) => {
      ...
      for (const ip of ips) {
        if (ip === "208.91.110.27") console.log(peer);
        map.set(ip.split(":")[0], peer.info?.name);
      }
  ...
  }, [getOptionMap, peers, transactions.txn_source_ipv4]);
  ...
  const incomeRankings = useMemo(() => {
    ...
    >((options, _, txnIdx) => {
      ...
      options.push({ txnIdx, income: getIncome(transactions, txnIdx) });
      return options;
    }, []);
    ...
    (txnIdx: number) => {
      ...
      if (chartEl && chartBorderEl && canvasEl) {
        if (!isElementFullyInView(canvasEl)) {
          ...
          console.log(canvasRect.top, headerRect?.bottom);
        ...
        chartBorderEl.style.border = "1px solid #1d6fba";
      ...
      uplotAction((u, _bankIdx) => {
        ...
        const desiredScaleTotal =
          (endTs - startTs) * desiredScaleTotalMultiplier;

        const notWithinScale = endTs < scaleMin || startTs > scaleMax;

        u.batch(() => {
          if (notWithinScale || scaleTotal > desiredScaleTotalMultiplier) {
            let min = Math.max(u.data[0][0], startTs - desiredScaleTotal / 2);
            const max = min + desiredScaleTotal;
            if (max > u.data[0][u.data[0].length - 1]) {
              min = max - desiredScaleTotal;
            }
    ...
    (inputValue: string, optionValue?: number | string) => {
      ...
      switch (searchMode) {
        case SearchMode.TxnSignature: {
          ...
          const searchIdx = { current: 0, total: txnIdxs.length - 1, txnIdxs };
        ...
        case SearchMode.Ip: {
          const option = ipOptionMap[inputValue];
          if (option) {
  ...
  const showList =
    (isOpen && !isQueryMode) ||
    (dInputValue.length >= searchLengthThreshold && !isLoading);
...
const TxnSigCommandItems = memo(function CommandItems({
  ...
  function filterOption([, { signatureLower }]: [
    ...
    return signatureLower.includes(searchValue);
  }
...
const IpCommandItems = memo(function CommandItems({
  ...
  function filterOption([, { label: ip, queryValue }]: [
    ...
    return !!findIpMatch(ip, searchValue) || queryValue.includes(searchValue);
  }

  function getRank([, { label: ip, txnIdxs }]: [string, IpCommandOption]) {
    if (ip === searchValue) return 100;
