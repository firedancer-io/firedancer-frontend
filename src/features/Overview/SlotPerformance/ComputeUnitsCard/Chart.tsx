  ...
  const events = [
    ...computeUnits.txn_start_timestamps_nanos.map((timestamp, i) => ({
      ...
      transaction: {
        ...
        computeUnitsEstimated: computeUnits.txn_compute_units_estimated[i],
        computeUnitsRebated: computeUnits.txn_compute_units_rebated[i],
...
function getSegments(
  maxComputeUnits: number,
  slotDurationNanos: number,
  bankTileCount: number,
  xDomain: Domain,
  yDomain: Domain,
) {
  const segments: Segment[][] = [];
  const adjustedMaxCUs = maxComputeUnits + 0.05 * slotDurationNanos * cusPerNs;

  const getCusAtTs = (ts: number, bankCount: number) => {
    return getCuByTs({
      ts,
      bankCount,
      tEnd: slotDurationNanos,
      maxComputeUnits: adjustedMaxCUs,
    });
  };
          ...
          return (
            ...
            >
              ...
              <YAxis
                ...
                domain={[
                  0,
                  (dataMax: number) => Math.max(dataMax + 1, 10000000),
                ]}
                // ticks={cuDomain ? undefined : defaultCuTicks}
                minTickGap={0}
                orientation="right"
