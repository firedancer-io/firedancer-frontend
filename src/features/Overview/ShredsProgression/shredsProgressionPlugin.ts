...
import { delayMs, shredColors, xRangeMs } from "./const";
    ...
    hooks: {
      drawSeries: [
        (u, sidx) => {
          ...
          const skippedSlots = store.get(skippedSlotsAtom);

          if (!liveShreds || !slotRange || !skippedSlots) {
            return;
          }
          ...
          uPlot.orient(
            ...
            (
              ...
              for (const slotNumber of orderedSlotNumbers) {
                ...
                const isSlotSkipped = skippedSlots.includes(slotNumber);
