...
import { createUnitQuad, type RgbColor } from "../../WebGl/webglUtils.ts";
import {
  viewMaxY,
  minHeightRatio,
  rowGapRatio,
...
float revenueAlpha(float value, float maxValue) {
  ...
  return clamp(r * r, MIN_ALPHA, MAX_ALPHA);
}
...
export function setTxnUniforms(
  txnMesh: TxnMesh,
  maxValue: number,
  minBarMs: number,
  rows: number,
  ...
  const { uniforms } = txnMesh.material;
