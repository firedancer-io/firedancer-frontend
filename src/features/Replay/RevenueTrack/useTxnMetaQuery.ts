...
const MIN_SUBWINDOW_NS = 1_000_000n;
...
export default function useTxnMetaQuery() {
  ...
  const setCache = useSetAtom(replayTxnMetaCacheAtom);
  const setMetaResponse = useSetAtom(replayTxnMetaResponseAtom);
  const setMetaError = useSetAtom(replayTxnMetaErrorAtom);
  const metaResponse = useAtomValue(replayTxnMetaResponseAtom);
  const metaError = useAtomValue(replayTxnMetaErrorAtom);
  ...
  const sendCurrent = useCallback(
    (run: BucketRun) => {
      ...
      setMetaResponse(undefined);
      setMetaError(undefined);

      if (timeoutRef.current) clearTimeout(timeoutRef.current);
      timeoutRef.current = setTimeout(() => {
        if (requestIdRef.current === reqId) setMetaError("timeout");
      }, QUERY_TIMEOUT_MS);
    ...
    [wsSend, setMetaResponse, setMetaError],
  );
  ...
  useEffect(() => {
    ...
    const settled = metaResponse !== undefined || metaError !== undefined;
    if (!settled) return;
    ...
    if (metaError === RESULT_LIMIT_EXCEEDED) {
      ...
      if (splittable) {
        ...
        run.current = null;
        logDebug("txnMeta", `split bucket ${run.bucketId} [${start}, ${end}]`);
        sendCurrent(run);
        return;
      }
      run.errorCode = RESULT_LIMIT_EXCEEDED;
    } else if (metaError !== undefined) {
      run.errorCode = metaError;
      logDebug(
        "txnMeta",
        `error bucket ${run.bucketId} [${start}, ${end}]: ${metaError}`,
      );
    } else if (metaResponse !== undefined) {
      appendTxnMeta(run.txns, metaResponse);
    }
    ...
    if (run.errorCode && !gotTxns) {
      ...
      const maxima = computeMaxima(run.txns);
      if (!bucketCacheRef.current.has(run.bucketId)) {
        orderRef.current.push(run.bucketId);
      }
      bucketCacheRef.current.set(run.bucketId, { txns: run.txns, maxima });
      publishCache();
      evict();
    }
