...
import { setMinDirtySlotByChartIfSmaller } from "../../Overview/ShredsProgression/atoms";
...
export function createShredAtoms() {
  ...
  return {
    ...
    addShredEvents: atom(null, (get, set, batch: HistoricalShreds[]) => {
      if (!batch.length) {
      ...
      set(_liveShredsAtom, (prev) => {
        const updated: SlotsShreds = prev ?? {
          referenceTs: Math.round(
            Number(batch[0].reference_ts / BigInt(nsPerMs)),
          ),
          slots: new Map(),
        };
...
function addEventToSlot(
  ...
  // reset if granularity reset
  const slot =
    !slotToMutate || slotToMutate.granularity !== granularity
      ? {
          granularity,
          shreds: [],
        }
