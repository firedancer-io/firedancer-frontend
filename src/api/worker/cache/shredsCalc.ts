...
import type { ValidatorStateCache } from "../types";
import type {
  LiveShredsData,
  ShredEventTsDeltaMs,
  ShredEventTsDeltas,
...
export function createShredsCalc(validatorStateCache: ValidatorStateCache) {
  ...
  function deleteSlots(isStartup: boolean, serverTimeNanos?: number) {
    ...
    const remainingSlotNumbers = data.slotsShreds.slots.keys();
    data.range =
  ...
  function setRecursiveDeleteTimeout() {
    timeoutId = setTimeout(
      () => {
        deleteSlots(
          !!validatorStateCache.isStartup,
          validatorStateCache.serverTimeNanos,
        );
