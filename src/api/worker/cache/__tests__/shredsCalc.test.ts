...
import { createLiveShredsCache } from "../shredsCache";
...
describe("createShredsCalc", () => {
  let liveShredsCache: ReturnType<typeof createLiveShredsCache> | undefined;
  ...
  it("adds live shred events for single shred, replacing duplicates with min ts and ignoring unsupported event types", () => {
    const subKey = "liveShreds1";
    liveShredsCache = createLiveShredsCache(vi.fn(), {});

    // before subscribe
    expect(liveShredsCache.get(subKey)).toBeUndefined();

    liveShredsCache.subscribe(subKey, {
      publishIntervalMs: Infinity,
    });

    // initial state after subscribe
    expect(liveShredsCache.get(subKey)).toEqual({});

    // add initial shreds
    liveShredsCache.add(subKey, {
      reference_slot: 2000,
  ...
  describe("non-startup deletion", () => {
    ...
    it("without server timestamp: deletes slot numbers before max completed slot number that was completed before chart min X", () => {
      ...
      // add initial shreds
      liveShredsCache.add(subKey, toAdd);

      expect(liveShredsCache.get(subKey)).toEqual(expectedInitial);

      // advance time to trigger deletion of old slots
      vi.advanceTimersByTime(POST_STARTUP_DELETE_INTERVAL_MS);

      expect(liveShredsCache.get(subKey)).toEqual(expectedPostDelete);
