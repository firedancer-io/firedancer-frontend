...
function flush() {
  ...
  if (items.length) {
    // latest kv type not implmented as everything defaults to batched kvb
    ctx.postMessage({ type: "kvb", items });
  }
}

function connect(url: string, zstd: ZstdDec | undefined, compress: boolean) {
  logDebug("WS", `Connecting to API WebSocket ${url.toString()}`);
  ctx.postMessage({ type: "connecting" });
  ws = new WebSocket(url, compress ? ["compress-zstd"] : undefined);
  ...
  switch (msg.type) {
    case "connect":
      void (async () => {
        ...
        connect(msg.websocketUrl, zstd, zstd ? (msg.compress ?? true) : false);
      })();
      break;
    case "disconnect":
      clearTimeout(reconnectTimer);
